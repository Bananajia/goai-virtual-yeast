...
本文件对应 GOAI 赛道三虚拟细胞方向关于开源范围、授权方式、第三方依赖、商业 API、闭源模型和数据来源的披露要求。它描述提交代码的实际状态，不把计划中的资源写成已经用于最终模型。

## 开放范围与授权

- 提交包中的小米蕉队原创 `research_code/` Python 源码、测试、配置和原创说明文档，随发布包根目录的 `LICENSE` 按 Apache License 2.0 授权。
- `research_code/evaluation/`、`pipeline/`、`SubmissionContract`、public-only Provider 合同、聚合证据 Adapter 和泄漏回归测试是本队拟形成的可复用开源贡献。
...
- 当前报告结果与 LIVE metadata Ridge **没有调用商业 API，也没有使用闭源模型**。
- 代码中存在一个默认关闭的 OpenAI-compatible public-only Provider 接口，仅作为未来实验 seam；它不参与当前提交结果，仓库不含 API key，也不要求评审调用或付费。
